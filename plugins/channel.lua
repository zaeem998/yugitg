...
if channel[tostring(v)][i] and getChatMembersCount("@"..channel[tostring(v)][i].username) < 200 then 
for kv,vv in pairs(channel.channel_user) do 
if vv == tostring(v) then 
...
if channel_dem ~= "" then 
sendMessage(main_group,"يتطلب 200 عضو للدخول \n" ..channel_dem.."سيتم ممسحها") 
end 
end 
...
if getChatMembersCount("@"..matches[3]) < 200 then 
sendMessage(msg.chat.id,"يتطلب 200 عضو للدخول") 
return 
end 
